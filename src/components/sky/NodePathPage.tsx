...
interface Props {
  ...
  nodes: PathNode[];
  lines: [number, number][];
  mapVariant?: "constellation" | "body";
  bodyMapBackground?: string | StaticImageData;
}
...
export function NodePathPage({ discipline, disciplineKey, nodes, lines, mapVariant = "constellation", bodyMapBackground }: Props) {
  ...
  useEffect(() => {
    ...
    setLiveNodes(nodes.map(n => ({
      ...n,
      status: computeStatus(disciplineKey, n.num),
    })));
  }, [disciplineKey, nodes]);
      ...
      <div className="content" style={{ paddingBottom: 100 }}>
              ...
              const marker = (
                <div style={{ position: "absolute", left: n.x + "%", top: n.y + "%", transform: "translate(-50%, -50%)", zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", pointerEvents: "auto" }}>
                  <div style={{ position: "relative", width: 62, height: 62, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {isCurrent && <div style={{ position: "absolute", inset: 1, borderRadius: "50%", boxShadow: "0 0 0 6px rgba(216,168,95,.13), 0 0 26px rgba(216,168,95,.58)" }} />}
                    <div style={{ width: nodeSize, height: nodeSize, borderRadius: "50%", border: (isCurrent ? "2.5px" : "1.5px") + " solid " + (isCurrent ? "rgba(244,204,136,.98)" : isDone ? "rgba(216,168,95,.72)" : "rgba(174,142,224,.5)"), background: isCurrent ? "radial-gradient(circle at 38% 32%, rgba(244,204,136,.34), rgba(84,32,150,.96))" : isDone ? "radial-gradient(circle at 38% 32%, rgba(216,168,95,.18), rgba(30,14,80,.96))" : "radial-gradient(circle at 38% 32%, rgba(255,255,255,.08), rgba(10,5,26,.96))", boxShadow: isCurrent ? "0 0 0 6px rgba(216,168,95,.1), 0 0 24px rgba(216,168,95,.5), 0 6px 16px rgba(0,0,0,.48)" : isDone ? "0 0 15px rgba(216,168,95,.26), 0 5px 13px rgba(0,0,0,.42)" : "0 5px 14px rgba(0,0,0,.58)", overflow: "hidden", position: "relative", opacity: isLocked ? 0.84 : 1 }}>
        ...
        <div style={{ position: "relative", height: MAP_H, marginBottom: 20 }}>
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 1 }}>
            ...
            {lines.map(([i, j], idx) => (
              <line key={idx} x1={liveNodes[i].x} y1={liveNodes[i].y} x2={liveNodes[j].x} y2={liveNodes[j].y} stroke="url(#lg1)" strokeWidth="0.5" strokeDasharray="2 1.5" strokeLinecap="round"/>
            ))}
            {lines.map(([i, j], idx) => {
              const mx = (liveNodes[i].x + liveNodes[j].x) / 2, my = (liveNodes[i].y + liveNodes[j].y) / 2;
              return (
            ...
            {liveNodes.map((n, i) => <circle key={"nc-" + i} cx={n.x} cy={n.y} r="0.55" fill="rgba(216,168,95,.5)"/>)}
          </svg>
        ...
        {/* No current node: all done */}
        {!currentNode && doneCount === liveNodes.length && (
          <div style={{ border: "1px solid rgba(216,168,95,.28)", borderRadius: 20, marginBottom: 10, padding: "22px 20px", textAlign: "center", background: "rgba(14,10,32,.6)", backdropFilter: "blur(12px)" }}>
            ...
            <p style={{ fontFamily: "var(--font-serif)", fontSize: 22, color: "var(--gold-2)" }}>
              {lang === "en" ? "Path Complete!" : "Put zavershen!"}
            </p>
          </div>
        )}
        ...
        <div style={{ border: "1px solid rgba(255,255,255,.08)", borderRadius: 16, padding: "14px 16px", background: "rgba(12,8,28,.45)", backdropFilter: "blur(6px)", WebkitBackdropFilter: "blur(6px)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <p style={{ fontSize: 12, color: "var(--muted)" }}>{doneCount} {t.nodePath.of} {liveNodes.length} {t.nodePath.nodes}</p>
          </div>
