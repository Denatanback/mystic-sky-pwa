...
export function ActivityCalendar({ state }: Props) {
  ...
  return (
    <section style={{ ...lunaCardStyle, padding: 16 }}>
      <p style={{ color: "var(--gold)", fontSize: 10, fontWeight: 900, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 10 }}>Последние 30 дней</p>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, color: "var(--muted)", fontSize: 11, fontWeight: 800 }}>
        <span>Активный</span>
        <span style={{ color: "var(--gold-2)" }}>Сегодня</span>
        <span style={{ color: "var(--muted-2)" }}>Пропущен</span>
      </div>
    </section>
  );
}
